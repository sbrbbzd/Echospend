...
const Assistant: React.FC = () => {
  const { expenses, budget, updateBudget } = useAppContext();
  ...
  const handleToolCalls = async (modelParts: any[], toolCalls: any[], originalMessage: string) => {
    ...
    for (const call of toolCalls) {
      ...
      if (call.name === 'getFinancialSummary') {
        ...
        updateBudget(call.args.newAmount);
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="flex items-center px-6 py-12 justify-between sticky top-0 bg-white/80 backdrop-blur-md z-30">
        <button onClick={() => router.back()} className="p-2 -ml-2 text-text-main hover:bg-gray-100 rounded-full transition-colors">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex items-start gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex flex-col gap-1 max-w-[80%] ${msg.role === 'user' ? 'items-end' : ''}`}>
              ...
              <div className={`p-4 rounded-[22px] shadow-sm border border-gray-100/60 text-[15px] leading-relaxed transition-all ${msg.role === 'assistant'
